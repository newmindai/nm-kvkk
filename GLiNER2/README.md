# GLiNER2 (modified copy for nm-kvkk)

> **This is not the official GLiNER2 package.** This folder is a modified copy of
> [fastino-ai/GLiNER2](https://github.com/fastino-ai/GLiNER2) at commit `3c913c7` (version 2.0.0), maintained by
> NewMind AI for the nm-kvkk project. It adds native tokenization for byte-level BPE encoders, a bf16 loss fix and a
> non-finite-gradient guard. [CHANGES.md](CHANGES.md) lists every change. Fastino AI does not maintain or endorse this copy.
> For the official package, use `pip install gliner2` and the upstream repository.
>
> The text below is the upstream README. For this copy, the installation, release and license sections are changed,
> and the section "Tokenization mode and BOS" is added.
...
This copy requires Python 3.10 or newer. It is not on PyPI. Do not install the PyPI package `gliner2` next to it:
the nm-kvkk models need the changes in this copy. Use it from the nm-kvkk repository root in one of these ways:

```bash
# The nm-kvkk recipes set this for you
export PYTHONPATH=GLiNER2

# Or install this copy in editable mode, with the same extras as upstream
pip install -e "GLiNER2[local]"   # local model inference and LoRA support
pip install -e "GLiNER2[train]"   # model training
pip install -e "GLiNER2[test]"    # tests
```
...
### Tokenization mode and BOS (`tokenization`, `add_bos`)

By default every word (and every schema marker) is passed to the encoder tokenizer on its own
(`tokenization="per_word"`). That is exact for SentencePiece encoders, whose tokenizer adds its own
word-start marker to any string, but byte-level BPE encoders (ModernBERT family) mark word starts
with a leading space merged into the piece, which a bare word never carries. For those encoders set
`ExtractorConfig(tokenization="native")`: each schema group and the text are tokenized as one
string, so the encoder sees the pieces it saw in pretraining (first word of a segment bare, later
words space-prefixed, markers atomic). `add_bos=True` prepends the encoder's BOS/CLS token. Both
settings are stored in `config.json` and restored by `from_pretrained`. `token_pooling="mean"` /
`"max"` are vectorised (`SchemaTransformer.pool_text_states`) and equal the reference loop.
...
This modified copy is not released as a package. Upstream releases follow the process in the
[upstream repository](https://github.com/fastino-ai/GLiNER2).

## 📄 License

This project is licensed under the Apache License 2.0 - see the [LICENSE](LICENSE) file for details.
The changes made by NewMind AI are listed in [CHANGES.md](CHANGES.md) and are under the same license.
