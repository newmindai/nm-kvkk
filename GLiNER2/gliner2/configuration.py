# Modified by NewMind AI in 2026 from fastino-ai/GLiNER2 commit 3c913c7: adds the tokenization and add_bos options.
...
class ExtractorConfig(PretrainedConfig):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        # How text/schema words are turned into encoder pieces: "per_word" (each word
        # tokenized alone; exact for SentencePiece encoders) or "native" (each segment
        # tokenized as one string, as the encoder saw running text; needed for
        # byte-level BPE encoders such as ModernBERT). add_bos prepends the encoder's
        # BOS token. Both are persisted in config.json.
        self.tokenization = str(tokenization or "per_word")
        if self.tokenization not in ("per_word", "native"):
            raise ValueError(
                f"tokenization must be 'per_word' or 'native', got {self.tokenization!r}"
            )
    ...
    def validate(self) -> None:
        ...
        if getattr(self, "tokenization", "per_word") not in ("per_word", "native"):
            raise ValueError(
                f"tokenization must be 'per_word' or 'native', got {self.tokenization!r}"
            )
        if self.architecture == "span":
