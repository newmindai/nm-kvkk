# Modified by NewMind AI in 2026 from fastino-ai/GLiNER2 commit 3c913c7: skips the optimizer step when the gradient norm is not finite.
...
class ExtractorTrainer:
    ...
    def _optimizer_step(self) -> bool:
        ...
        # A non-finite total norm means at least one gradient is NaN/Inf and
        # clip_grad_norm_ has already multiplied EVERY gradient by that norm:
        # stepping now would poison all parameters irreversibly (observed on
        # H100/bf16: one bad backward froze a whole 3000-step run at loss 0).
        # Skip the step, keep the schedule aligned, and say so loudly.
        if isinstance(grad_norm, torch.Tensor) and not torch.isfinite(grad_norm).all():
            logger.warning(
                "Non-finite gradient norm at step %d; skipping optimizer step",
                self.global_step,
            )
