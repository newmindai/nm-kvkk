# Modified by NewMind AI in 2026 from fastino-ai/GLiNER2 commit 3c913c7: passes the tokenization and add_bos settings to the processor.
...
class BoundaryExtractorModel(BaseExtractorModel):
    ...
    def __init__(
        ...
        if tokenizer is not None:
            self.processor = SchemaTransformer(
                ...
                tokenization=getattr(config, "tokenization", "per_word"),
                add_bos=getattr(config, "add_bos", False),
            )
        else:
            self.processor = SchemaTransformer(
                config.model_name,
    ...
    def _encode_core(self, batch) -> Dict[str, Any]:
        ...
        fast_routing = (
            (getattr(self.processor, "token_pooling", "first") == "first"
             or getattr(batch, "text_word_ids", None) is not None)
        ...
        if fast_routing:
            ...
            text_states = (
                ...
                gather_routed(batch.text_word_indices, batch.text_word_mask)

                if getattr(self.processor, "token_pooling", "first") == "first"

                else self.processor.pool_text_states(token_embeddings, batch)
