# Modified by NewMind AI in 2026 from fastino-ai/GLiNER2 commit 3c913c7: makes marginal_pair_consistency_loss finite in bf16.
...
def marginal_pair_consistency_loss(
    ...
    # log(1 - sigmoid(x)) computed as logsigmoid(-x): finite value AND bounded
    # gradient (-sigmoid(x)) in every dtype. The previous form,
    # log1p(-sigmoid(x).clamp(max=1.0 - 1e-6)), loses its clamp under bf16
    # (1.0 - 1e-6 rounds to exactly 1.0), so a saturated sigmoid reaches
    # log1p(-1.0): the forward stays finite but backward mints NaN gradients.
    # Computed in fp32 so bf16 autocast cannot degrade the survival sums.
    log_survival = F.logsigmoid(-pair_logits.float()) * valid_mask.to(torch.float32)
